...
> 마지막 업데이트: 2026-05-16 (교수님 피드백 반영 — AI 대리인 봇 / 논문 탭 추가)
...
> 5.5 교수님 AI 대리인 봇 / 5.6 교수님 논문 탭 신규 추가 (교수님 피드백 반영)
...
> 학생이 기회를 찾아다니는 것이 아니라, **기회가 먼저 학생을 찾아오는 구조**의 학과 AI 매칭 플랫폼

### 1.2 기획 배경

- 교수님 연구실 인턴/캡스톤 공고가 단톡에 묻혀서 학생이 기회를 놓침
- 연구실 선택 시 선배들의 커리어 정보를 알 수 없음
- 에브리타임, 학교 포털 어디에도 없는 학과 맞춤 기능 부재
- 선배한테 물어보고 싶어도 누구한테 물어봐야 할지 모름
...
| 교수님 랭킹 | 학생 투표 기반 랭킹 (탑 10) - 추후 가중치 항목 추가 예정 |
| 채팅 | 교수님 AI 봇 (1차) → 교수님 직접 채팅 (2차) / 연구생↔후배 / 이용자↔이용자 |
| 교수님 AI 대리인 봇 | 교수님 문서 업로드 → Gemini RAG → 학생 질문에 AI가 먼저 답변 |
| 교수님 논문 탭 | Semantic Scholar API 연동으로 교수님 논문 목록 자동 표시 |
...
| 메인 DB | Supabase (PostgreSQL) | 회원/공고/지원/랭킹 데이터 |
| 채팅 DB | Firebase Firestore | 실시간 채팅 |
...
```
1단계  교수님 공고 등록 (직접 or 관리자 대신)
   ↓
2단계  Gemini AI가 조건 + 성향 분석 → 학생 DB 필터링
   ↓
3단계  조건 맞는 학생에게만 푸시 알림 or 직접 조건 검색
   ↓
4단계  학생이 미리 입력한 프로필로 바로 지원
   ↓
5단계  교수님 지원자 카드 비교 + AI 추천 순위 확인
   ↓
6단계  수락/거절 → 결과 알림 + 채팅방 자동 생성 + AI 피드백 제공
```

### 5.2 연구실 커리어 트래킹

- 교수님별 출신 선배 익명 포트폴리오 제공
- 졸업 후 취업/대학원 진로 데이터 시각화
- 현재 연구생 정보 포함 (재학생은 이미 앱 가입)
- 졸업 선배는 추후 유입 전략 별도 수립 예정

### 5.3 교수님 랭킹

- 학생 투표 기반 탑 10 랭킹 출력
- 투표 → 가중치 계산 → 정렬 → 탑 10 표시
- 랭킹 세부 기준 및 가중치 항목 → 추후 결정

### 5.4 채팅

> 학생이 교수님을 만나기 전에, 먼저 교수님의 AI를 만납니다.

**채팅 레이어 구조**

```
학생 → 교수님 AI 봇 (1차)
         ↓ 봇이 해결 못하거나 학생이 직접 연결 요청 시
학생 → 교수님 직접 채팅 (2차)
```

| 채팅 유형 | 조건 | 설명 |
|----------|------|------|
| 학생 ↔ 교수님 AI 봇 | 교수님 프로필 진입 시 | 교수님 문서 기반 AI가 먼저 질문 응대 |
| 교수님 ↔ 학생 | 매칭 수락 후 자동 개설 | 지원 수락 시 채팅방 자동 생성 |
...
**배경**
교수님이 학생과 1시간 넘게 상담했는데 학생이 "생각이 바뀌었어요" 메일을 보내는 상황 발생 → 학생이 교수님 연구 분야를 제대로 모르고 찾아오는 게 원인

**구조 (RAG 기반)**

```
1단계  교수님이 문서 업로드 (논문 PDF, 연구 요약, FAQ 등)
   ↓
2단계  Gemini API가 문서 내용 기반으로 질문 응대
   ↓
3단계  학생이 교수님 프로필에서 AI 봇에게 먼저 질문
   ↓
4단계  교수님은 주기적으로 봇 대화 로그 확인
   ↓
5단계  잘못된 답변 발견 시 문서 재업로드로 재학습
```
...
**학생 화면**
- 교수님 프로필 내 "AI 봇에게 질문하기" 버튼
- 봇 대화 후 "교수님께 직접 연결" 버튼 (2차 채팅으로 전환)

---

### 5.6 교수님 논문 탭

- Semantic Scholar API 연동 (공식 무료 API, 구글 스칼라 대체)
- 교수님 이름으로 논문 목록 자동 조회
- 논문 제목 / 연도 / 인용 수 표시
- 논문 클릭 시 원문 링크 이동
- 교수님이 직접 논문 PDF 업로드도 가능 (AI 봇 학습 연동)
...
### professor_docs (AI 봇 학습 문서)
```sql
id            UUID PRIMARY KEY
professor_id  UUID REFERENCES professors(id)
file_name     VARCHAR   -- 업로드 파일명
file_url      VARCHAR   -- 저장 경로 (Supabase Storage)
doc_type      VARCHAR   -- 'paper' | 'summary' | 'faq' | 'other'
is_active     BOOLEAN   -- 봇 학습 활성화 여부
created_at    TIMESTAMP
```

### bot_conversations (AI 봇 대화 로그)
```sql
id            UUID PRIMARY KEY
professor_id  UUID REFERENCES professors(id)
student_id    UUID REFERENCES users(id)
question      TEXT      -- 학생 질문
answer        TEXT      -- AI 봇 답변
is_escalated  BOOLEAN   -- 교수님 직접 채팅으로 전환 여부
created_at    TIMESTAMP
```
...
### 교수님
```
GET    /api/professors          교수님 목록
GET    /api/professors/:id      교수님 상세
POST   /api/professors          교수님 등록 (관리자)
PUT    /api/professors/:id      교수님 수정
GET    /api/professors/:id/career  커리어 트래킹 조회
GET    /api/professors/ranking  랭킹 탑 10
POST   /api/professors/:id/vote 랭킹 투표
```
...
### 교수님 AI 봇
```
POST   /api/bot/:professorId/chat     학생 → AI 봇 질문
  body: { student_id, question }
  return: { answer, is_escalated }

GET    /api/bot/:professorId/logs     봇 대화 로그 조회 (교수님)
POST   /api/bot/:professorId/docs     문서 업로드 (교수님)
DELETE /api/bot/:professorId/docs/:id 문서 삭제 (교수님)
PUT    /api/bot/:professorId/docs/:id 문서 활성화/비활성화 (교수님)
```

### 논문 탭
```
GET    /api/professors/:id/papers     Semantic Scholar API 연동 논문 목록
  query: { name }
  return: { papers: [{ title, year, citationCount, url }] }
```
...
│   │   ├── posts/manage/page.jsx
│   │   ├── applicants/[postId]/page.jsx
│   │   ├── bot/page.jsx            ← 봇 문서 관리 + 대화 로그
│   │   └── papers/page.jsx         ← 논문 목록
│   ├── bot/
│   │   └── [professorId]/page.jsx  ← 학생 → AI 봇 채팅 화면
│   ├── career/page.jsx
│   ├── ranking/page.jsx
│   ├── chat/
│   │   ├── page.jsx
...
│   ├── ApplicantCard.jsx
│   ├── ProfessorCard.jsx
│   ├── BotChatWindow.jsx           ← AI 봇 채팅 UI 컴포넌트
│   └── Navbar.jsx
└── utils/
    ├── supabase.js
    ├── firebase.js
...
### 9.3 교수님 AI 대리인 봇 프롬프트

```
[시스템]
당신은 {professor_name} 교수님의 AI 대리인입니다.
아래 교수님이 제공한 문서를 기반으로만 답변하세요.
...
can_answer가 false이거나 suggest_escalation이 true면
프론트에서 "교수님께 직접 연결하기" 버튼 표시
```
...
> 채팅방 생성 조건
> - professor_student: 지원 수락 시 자동 생성 (AI 봇 응대 후 직접 연결 요청 시에도 생성 가능)
> - lab_junior: 연구생 등록 시 생성
> - user_user: 이용자가 직접 개설

> ※ AI 봇 대화는 Firebase가 아닌 Supabase bot_conversations 테이블에 저장
> Firebase는 사람↔사람 실시간 채팅에만 사용
...
| 항목 | 내용 | 결정 시기 |
|------|------|----------|
| 교수님 랭킹 기준 | 가중치 항목 세부 결정 필요 | 1일차 오전 회의 |
| 졸업 선배 유입 전략 | 앱 가입 유도 방법 | 추후 결정 |
| 채팅 기능 최종 확정 | Firebase 연동 여부 | 1일차 오전 회의 |
| 학생↔학생 채팅 범위 | 누구랑 누구까지 가능한지 정의 | 1일차 오전 회의 |
